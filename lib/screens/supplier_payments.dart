...
class _SupplierPaymentsPageState extends State<SupplierPaymentsPage> {
  ...
  Future<void> _loadPayments() async {
    ...
    try {
      final payments = await _supplierService.getPayments();
      setState(() {
        _payments = payments;
        _isLoading = false;
      });
  ...
  Future<void> _makePayment(Map<String, dynamic> order) async {
    try {
      print('=== MAKING PAYMENT WITH RAZORPAY SERVICE (SUPPLIER) ===');
      ...
      _razorpayService.initialize(
        onPaymentSuccess: (PaymentSuccessResponse response) async {
          ...
          try {
            ...
            await _supplierService.updateOrderStatusAfterPayment(
      ...
      await _razorpayService.openPaymentGateway(
        ...
        customerName: order['supplier_name'] ?? 'Supplier',
        customerEmail: order['supplier_email'] ?? 'supplier@example.com',
        customerPhone: order['supplier_phone'] ?? '+91 9876543210',
